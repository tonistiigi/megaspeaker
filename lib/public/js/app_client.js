...
$(function() {
  _.extend(ms, {
    ...
    startPlayer: function(e) {
      ...
      var url =  'http://commondatastorage.googleapis.com/megaspeaker/silence.mp3';
      if(/iphone|ipad|ipod/i.test(window.navigator.userAgent)){
        url = url.replace(/mp3$/, 'caf');
      }
      ms.spkr = new Spkr(e, url, 4);
    ...
    onLoadTrack: function(track){
      ...
      ms.track = track;
      if(!ms.spkr) return;
      //console.log('load', track.resources[1], track.duration);
      
      if(/ipad|iphone|ipod/i.test(window.navigator.userAgent) && track.resources[0].length) {
        ms.spkr.loadFile(track.resources[0], track.duration);
      }
      else {
        ms.spkr.loadFile(track.resources[1], track.duration);
      }
      $("#title").html("Buffering track...");
    },
  ...
  ms.on('ready', function() {
    //console.log('emit_ready');
    ms.socket.emit('playerReady', 123);
    $("#playBtn").hide();
    $("#speaker-on").show();
  ...
  ms.on('loaded', function(track) {
    ...
    $("#title").html('Speaker activated, please stand by.');
  });
