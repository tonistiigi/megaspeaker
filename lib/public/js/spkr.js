...
var Spkr = function(e, src, duration){
  ...
  _.bindAll(this, 'step', 'onCanPlayThrough', 'setCurrentTimeAsync', 'onPlay', 'onPause', 'onCanPlay');
...
_.extend(Spkr.prototype, {
  ...
  playAt: function(position, correction, expectedLatency){
    ...
    log('playat', position);
    this.seeking = true;
    var setTime = this.setCurrentTimeAsync(position, function(err){
      /*
      if(err){
        self.audio.play();
        setTimeout(function(){
          self.playAt(position, correction, expectedLatency);
        },500);
      }
      else {*/
        self.seeking = false;
  ...
  setCurrentTimeAsync: function(value, cb, retry) {
    ...
    if(this.isseeking) {
      return setTimeout(function(){
        this.setCurrentTimeAsync(value, cb, retry);
      }, 30);
    }
    ...
    if (false && ua.ios) {
      log('settimeasync');
      if(Math.abs(this.audio.currentTime- value)<0.01) {
        return cb();
      }
      /*if (!retry) retry = 0;
      if(retry>2){
      ...
      cb(!this.audio.currentTime && value>0);
