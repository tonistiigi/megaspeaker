...
app.get('/:name/master', function(req, res, next) {
  ...
  if(!selectedChannel) {
    channels[req.params.name] = { trackToLoad: false };
    req.url = '/master.html';
  }
  else {
...
io.of('/master').on('connection', function(socket) {
  ...
  socket.on('stop', function(trackId) {
    socket.get('channel', function(err, name) {
      ...
      io.of('/client').in(name).emit('stop', payload);
    });
  });
...
io.of('/client').on('connection', function(socket) {
  socket.on('register', function(data) {
    ...
    if(channels[data.channel] && channels[data.channel].trackToLoad) {
      socket.broadcast.to(data.channel).emit(
        'load',
        CONF.tracks[channels[data.channel].trackToLoad]
      );
    }
  });
