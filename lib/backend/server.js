...
io.of('/master').on('connection', function(socket) {
  socket.on('register', function(data) {
    ...
    channels[data.channel] = { trackToLoad: false, loaded: 0 };
  ...
  socket.on('load', function(trackId) {
    socket.get('channel', function(err, name) {
      ...
      channels[name].trackToLoad = trackId;
...
io.of('/client').on('connection', function(socket) {
  ...
  socket.on('playerReady', function(){
    if(channels[data.channel] && channels[data.channel].trackToLoad) {
      socket.emit('load', CONF.tracks[channels[data.channel].trackToLoad]);
      if(channels[data.channel].lastPayload){
        socket.emit('payload', CONF.tracks[channels[data.channel].lastPayload]);
      }
    }
  });
  ...
  socket.on('loaded', function(trackId) {
    socket.get('channel', function(err, name) {
      ...
      socket.broadcast.to(name).emit('loadedCount', channels[name].loaded);
  ...
  socket.on('disconnect', function() {
    socket.get('channel', function(err, name) {
      io.of('/master').in(name).emit('speakerCount',
        io.of('/client').clients(name).length - 1);
    });
  });
