...
io.of('/client').on('connection', function(socket) {
  socket.on('register', function(data) {
    socket.join(data.channel);
    socket.set('channel', data.channel);
    io.sockets.in(data.channel).emit('speakerCount',
  ...
  socket.on('loaded', function(trackId) {
    socket.get('channel', function(err, name) {
      ...
      io.sockets.in(name).emit('loadedCount', channels[name].loaded);
  ...
  socket.on('disconnect', function() {
    socket.get('channel', function(err, name) {
      ...
      io.sockets.in(name).emit('speakerCount', channels[name].loaded);
    });
  });
